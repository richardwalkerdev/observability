...
Everything should be the same except now the configuration is decoupled from any instance of Prometheus. When the second instance is added in this example `prometheus_targets.yml` should include both instacnes:
...
Everything from this point on involves adding target scrape configurations and rules, specifically alert rules. All the other components are *peripheral* to Prometheus, either extending or handing off services, or consuming data for other purposes, as in the case of Grafana that using Promethues as a data source for displaying information in a graphical way. 
